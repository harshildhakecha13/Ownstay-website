    ...
    <section className="section-pad bg-background" id="how-it-works" aria-labelledby="hiw-heading">
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center max-w-xl mx-auto mb-16">
          <p className="text-xs font-bold tracking-[0.3em] uppercase text-primary mb-4">How It Works</p>
          <h2 id="hiw-heading" className="text-section-xl font-bold text-foreground tracking-tight mb-5">
        ...
        <div ref={ref} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 relative">
          ...
          <div className="absolute top-10 left-[12.5%] right-[12.5%] h-0.5 bg-border hidden lg:block" aria-hidden="true" />

          {steps.map((step) => (
            <div
              key={step.number}
              className="step-item relative bg-card border border-border rounded-2xl p-7 shadow-card"
              style={{ opacity: 1, transform: 'translateY(0)', transition: 'all 0.6s cubic-bezier(0.16, 1, 0.3, 1)' }}
            >
              {/* Number badge */}
              <div className="flex items-center gap-3 mb-5">
                <div className="w-10 h-10 rounded-xl bg-primary flex items-center justify-center flex-shrink-0 z-10 relative">
                  <Icon name={step.icon as Parameters<typeof Icon>[0]['name']} size={18} className="text-primary-foreground" />
                </div>
                <span className="text-xs font-bold tracking-widest text-muted-foreground">{step.number}</span>
              </div>
