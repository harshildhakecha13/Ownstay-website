        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="order-2 lg:order-1">
            <div className="bg-card rounded-3xl border border-border shadow-card-hover overflow-hidden">
              ...
              <div className="p-5 space-y-4 bg-secondary/30">
                {[
                  { from: 'ai', text: 'Hello! Welcome to Meridian Resort. How can I assist you today?' },
                  { from: 'guest', text: 'Hi! What time does the spa open?' },
                  { from: 'ai', text: 'Our spa is open daily from 9:00 AM to 9:00 PM. Would you like to make a booking?' },
                  { from: 'guest', text: "Yes, for tomorrow at 3 PM for 2 people." },
                  { from: 'ai', text: "I've noted your spa booking request for 2 guests at 3:00 PM tomorrow. Our spa team will confirm shortly. Is there anything else?" },
                ]?.map((msg, i) => (
                  <div key={i} className={`flex ${msg?.from === 'guest' ? 'justify-end' : 'justify-start'}`}>
                    ...
                    <div className={`max-w-[78%] px-4 py-3 rounded-2xl text-sm leading-relaxed ${
                      msg?.from === 'guest' ?'bg-foreground text-white rounded-tr-sm' :'bg-card border border-border text-foreground rounded-tl-sm shadow-card'
                    }`}>
                      {msg?.text}
                    </div>
                  </div>
              ...
              <div className="px-5 py-4 border-t border-border bg-card">
                <div className="flex items-center gap-3 bg-secondary rounded-xl px-4 py-3">
                  ...
                  <button className="w-7 h-7 rounded-lg bg-primary flex items-center justify-center" aria-label="Send">
                    <Icon name="PaperAirplaneIcon" size={13} className="text-primary-foreground" variant="solid" />
                  </button>
                </div>
              </div>
            </div>
          ...
          <div className="order-1 lg:order-2">
            <div className="inline-flex items-center gap-2 bg-blue-50 border border-blue-100 rounded-full px-3 py-1.5 mb-6">
              ...
              <span className="text-xs font-bold tracking-widest uppercase text-blue-600">Guest Chat</span>
            </div>
            <h2 id="chat-heading" className="text-section-xl font-bold text-foreground tracking-tight mb-5">
              AI Guest Chat
            </h2>
            <p className="text-base text-muted-foreground leading-relaxed mb-8">
              Guests communicate with Ownstay through your website, mobile app, or any digital channel. Conversations are intelligent, on-brand, and always available.
            </p>
