...
const aiAlerts = [
  {
    ...
    type: 'warning',
    icon: '🧹',
    msg: 'Room 204 — Housekeeping delayed 28 min past scheduled clean. Reassign?',
  },
  {
    time: '13:58',
    type: 'critical',
    icon: '🍳',
    msg: 'Kitchen load at 92% capacity. Indian/Tandoor station overloaded.',
  },
  {
    time: '13:45',
    type: 'info',
    icon: '📋',
    msg: 'Guest Ananya Sharma (Suite 402) requested late checkout — 2 PM.',
  },
  {
    time: '13:31',
    type: 'warning',
    icon: '📦',
    msg: 'Norwegian Salmon at 0kg. Supplier requisition required before dinner.',
  },
  {
    time: '13:15',
    type: 'info',
    icon: '🔑',
    msg: '9 new arrivals confirmed. Digital keys pre-issued to 7 guests.',
  },
];
        ...
        <div className="flex px-6 py-3 border-b border-gray-50 gap-1">
          {wizardSteps.map((ws) => (
            <div key={ws.no} className="flex-1 flex flex-col items-center gap-1">
              <div
                className={`w-7 h-7 rounded-full flex items-center justify-center text-[11px] font-bold transition-all ${step > ws.no ? 'bg-green-500 text-white' : step === ws.no ? 'bg-orange-500 text-white' : 'bg-gray-100 text-gray-400'}`}
              >
                {step > ws.no ? '✓' : ws.no}
              </div>
              <div
                className={`text-[9px] font-semibold text-center leading-tight ${step === ws.no ? 'text-orange-500' : step > ws.no ? 'text-green-600' : 'text-gray-400'}`}
              >
                {ws.title}
              </div>
            </div>
        ...
        <div className="px-6 py-5 min-h-[180px]">
            ...
            <div>
              ...
              {guestSearch.length > 1 && (
                <div className="mt-2 border border-gray-100 rounded-xl overflow-hidden">
                  {[
                    'Rohan Verma — #BK-20240902 — Suite 301',
                    'Ananya Sharma — #BK-20240891 — Suite 402',
                  ].map((r) => (
                    <div
                      key={r}
                      onClick={() => setGuestSearch(r.split(' — ')[0])}
                      className="px-4 py-2.5 text-sm hover:bg-orange-50 cursor-pointer border-b border-gray-50 last:border-0 font-medium text-gray-700"
                    >
                      {r}
                    </div>
                  ))}
            ...
            <div>
              ...
              <div className="bg-gray-50 rounded-xl p-4 border border-gray-100 flex gap-4 items-center">
                ...
                <div className="flex flex-col gap-2">
                  {['Photo matches guest', 'Document not expired', 'Address captured'].map(
                    (item) => (
                      <label
                        key={item}
          ...
          {step === 3 && (
            <div>
              ...
              <div className="grid grid-cols-3 gap-2 mb-3">
                {['Suite 301', 'Suite 305', 'Double 201', 'Single 105', 'Suite 402 ✦'].map((r) => (
                  <button
                    ...
                    className={`rounded-xl border py-2.5 text-xs font-semibold transition-all ${selectedRoom === r ? 'border-orange-400 bg-orange-50 text-orange-600' : 'border-gray-200 bg-white text-gray-600'}`}
                  >
                    {r}
                  </button>
                ))}
              ...
              {selectedRoom && (
                <div className="bg-green-50 border border-green-200 rounded-xl px-4 py-2 text-xs text-green-700 font-medium">
                  🔑 Digital key issued to guest&apos;s registered mobile.
                </div>
              )}
            </div>
          )}
          {step === 4 && (
            <div>
              ...
              <div className="bg-gray-50 border border-gray-100 rounded-xl p-3 text-xs text-gray-500 leading-relaxed h-24 overflow-y-auto mb-3">
                The guest acknowledges check-in policies and hotel rules. By proceeding, the guest
                agrees to the property&apos;s terms including no-smoking policy, responsibility for
                damages, and check-out time of 11:00 AM unless late checkout has been confirmed...
              </div>
              <label className="flex items-center gap-3 cursor-pointer">
                <div
                  onClick={() => setAgreed(!agreed)}
                  className={`w-10 h-5 rounded-full transition-all relative ${agreed ? 'bg-orange-500' : 'bg-gray-200'}`}
                >
                  <div
                    className={`absolute top-0.5 w-4 h-4 bg-white rounded-full shadow transition-all ${agreed ? 'left-5' : 'left-0.5'}`}
                  />
                </div>
          ...
          {step === 5 && (
            <div className="flex flex-col items-center py-4 text-center gap-3">
              ...
              <div className="flex gap-2">
                <div className="bg-orange-50 border border-orange-200 rounded-lg px-3 py-1.5 text-xs font-semibold text-orange-600">
                  🔑 Key Issued
                </div>
                <div className="bg-green-50 border border-green-200 rounded-lg px-3 py-1.5 text-xs font-semibold text-green-600">
                  ✅ Folio Created
                </div>
                <div className="bg-blue-50 border border-blue-200 rounded-lg px-3 py-1.5 text-xs font-semibold text-blue-600">
                  📱 SMS Sent
                </div>
              </div>
              <button
                onClick={onClose}
                className="bg-orange-500 text-white px-6 py-2 rounded-xl font-bold text-sm hover:bg-orange-600 transition-colors"
              >
                Done
              </button>
            </div>
        ...
        {step < 5 && (
          <div className="flex items-center justify-between px-6 py-3 border-t border-gray-100 bg-gray-50">
            <button
              ...
              disabled={step === 1}
              className="text-sm font-semibold text-gray-400 hover:text-gray-600 disabled:opacity-30"
            >
              ← Back
            </button>
            ...
            <button
              ...
              className="bg-orange-500 text-white px-5 py-2 rounded-xl font-bold text-sm hover:bg-orange-600 transition-colors"
            >
      ...
      <div className="max-w-[1400px] mx-auto px-6 py-8 flex flex-col gap-8">
        ...
        <section>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3">
            {kpiData.map((k) => (
              <div
                key={k.label}
                className="bg-white rounded-2xl p-4 flex flex-col gap-1 border border-gray-100"
              ...
              >
                <div className="flex items-start justify-between">
                  ...
                  {k.trend && (
                    <span
                      className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${k.trendUp ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-500'}`}
                    >
                      {k.trend}
                    </span>
                  )}
                </div>
                <div className="text-xl font-bold mt-1" style={{ color: k.color || '#1A1A1A' }}>
                  {k.value}
                </div>
        ...
        <section>
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="bg-orange-50 border border-orange-200 rounded-full px-2 py-0.5 text-[10px] font-bold text-orange-600">
              ✦ AI Active
            </div>
          </div>
          <div
          ...
          >
            {aiAlerts.map((a, i) => (
              <div
                key={i}
                className={`flex items-start gap-4 px-5 py-3 border-b border-gray-50 last:border-0 transition-all duration-500 ${i === alertIdx ? 'bg-orange-50/60' : ''}`}
              >
                <span className="text-base shrink-0">{a.icon}</span>
                <p className="text-sm text-gray-700 flex-1 leading-snug">{a.msg}</p>
                <div className="flex items-center gap-2 shrink-0">
                  <span
                    className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${a.type === 'critical' ? 'bg-red-100 text-red-700' : a.type === 'warning' ? 'bg-amber-100 text-amber-700' : 'bg-blue-100 text-blue-700'}`}
                  >
                    {a.type}
                  </span>
        ...
        <section>
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-[11px] font-bold uppercase tracking-[0.2em] text-gray-400">
              Front Desk — Room Gantt Timeline
            </h2>
            <div className="text-xs text-gray-400">Week of Sep 2–8, 2024</div>
          </div>
          <div
            className="bg-white rounded-2xl border border-gray-100 p-6 overflow-x-auto"
            style={{ boxShadow: '0 4px 20px rgba(0,0,0,0.04)' }}
          >
            <div style={{ minWidth: colW * 7 + 160 }}>
              <div className="flex mb-1">
                ...
                {days.map((d) => (
                  <div
                    key={d}
                    className="text-center text-[10px] font-bold uppercase tracking-wider text-gray-400 pb-2 border-b border-gray-100"
                    style={{ width: colW }}
                  >
                    {d}
                  </div>
              ...
              {rooms.map((room) => {
                ...
                const blockSpan =
                  room.status === 'clean' || room.status === 'maintenance'
                    ? 1
                    : Math.max(1, room.checkOut - room.checkIn);
                return (
                  <div
                    key={room.roomId}
                    className="flex items-center border-b border-gray-50 py-1"
                    style={{ height: 40 }}
                  >
                    <div className="w-40 shrink-0 text-xs font-semibold text-gray-600 pr-2 truncate">
                      {room.roomId}
                    </div>
                    <div className="flex relative" style={{ width: colW * 7 }}>
                      {days.map((_, idx) => (
                        <div
                          ...
                          style={{ width: colW, height: 30 }}
                        />
                      ))}
                      {room.status !== 'clean' && (
                        <div
                          className="absolute top-0.5 rounded-lg flex items-center px-2 text-[10px] font-semibold border truncate"
                          style={{
                            left: room.checkIn * colW,
                            width: Math.max(blockSpan, 1) * colW - 4,
                            height: 26,
                            background: s.bg,
                      ...
                      {room.status === 'clean' && (
                        <div
                          className="absolute top-0.5 rounded-lg flex items-center px-2 text-[10px] font-semibold border"
                          style={{
                            left: 0,
                            width: colW * 7 - 4,
                            height: 26,
        ...
        <div className="bg-gradient-to-r from-orange-500 to-orange-600 rounded-2xl p-6 flex items-center justify-between text-white">
          <div>
            ...
            <p className="text-sm opacity-80">
              Use the 5-step check-in wizard to assign rooms and issue digital keys.
            </p>
          </div>
          <button
            onClick={() => setWizardOpen(true)}
            className="bg-white text-orange-600 font-bold px-5 py-2.5 rounded-xl text-sm hover:bg-orange-50 transition-colors shrink-0"
          >
            Start Check-In →
          </button>
        </div>
